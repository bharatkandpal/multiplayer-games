...
| UI-5  | MPG-112 | Tag filter row on Home reading the same `gameTags()` output                                                                              | `HomeScreen.tsx` + `.module.css`                                                     | S    | ✅ `38e25a9`            |
| UI-6  | MPG-113 | Extract a shared `SeatRail`; board surfaces move onto the table ground                                                                   | `components/ui/SeatRail` · `GamePlayScreen` · `tokens.css`                           | M    | ✅ shipped              |
...
| UI-9  | MPG-116 | Tier-2 material tokens: promote Connect Four's fifteen `rgba()` literals to `--game-*`. Pure refactor, pixel-identical                   | `tokens.css` · `ConnectFourBoard.module.css`                                         | S    | ✅ `d1a8591`            |
| UI-10 | MPG-117 | Shared `BoardGrid` primitive; all five boards onto one cell grammar. Kills radius/gap drift                                              | `components/board/BoardGrid` · all five `*Board.module.css`                          | M    | ✅ `9627517`            |
